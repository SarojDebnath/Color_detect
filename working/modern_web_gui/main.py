#!/usr/bin/env python3
"""
Modern Web GUI for LED Test Controller
...
app = FastAPI(title="LED Test Controller - Modern Web GUI", version="1.0.0", lifespan=lifespan)
...
def update_server_status():
    ...
    try:
        ...
        if response.status_code == 200:
            server_status = {"running": True, "cameras_connected": 1}
        else:
            server_status = {"running": False, "cameras_connected": 0}
    except:
        server_status = {"running": False, "cameras_connected": 0}
...
class ModernLEDTestController(LEDTestController):
    ...
    def login_to_device(self):
        """Login to the Linux device - IMPROVED with better error handling"""
        ...
        add_console_message(f"Initial output: {output.strip()}", "info")
        ...
        if "password:" in output.lower():
            ...
            output = self.read_serial_output(15)  # Wait longer for login result
    ...
    def run_automated_test_web(self):
        ...
        try:
            add_console_message("Starting LED Test Controller", "info")
            update_test_status(True, "Initializing...", False, "")
            
            if not self.connect_serial():
                add_console_message("Failed to connect to serial port", "error")
                update_test_status(False, "Failed - Serial connection error", False, "")
                return False
            ...
            if not self.login_to_device():
                ...
                update_test_status(False, "Failed - Login error", False, "")
                return False
                ...
                if "[yes/no/skip]:" in output.lower() or "[yes/no/skip/exit]:" in output.lower():
                    ...
                    if "are all the leds red?" in output.lower():
                        ...
                        if all_red:
                            ...
                            add_console_message(f"❌ FAIL: Found {len(failed_red_data)} non-red LEDs", "error")
                        ...
                        if all_green:
                            ...
                            add_console_message(f"❌ FAIL: Found {len(failed_green_data)} non-green LEDs", "error")
                        ...
                        if all_blue:
                            ...
                            add_console_message(f"❌ FAIL: Found {len(failed_blue_data)} non-blue ETH LEDs", "error")
            ...
            update_test_status(False, "Test finished", False, "")
...
async def get_main_page():
    ...
    return FileResponse("modern_web_gui/templates/index.html")
...
# Serve static files
app.mount("/static", StaticFiles(directory="modern_web_gui/static"), name="static")
