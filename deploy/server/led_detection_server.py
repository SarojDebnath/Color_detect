...
# Enhanced best image tracking (last 5 seconds with quality assessment)
best_image_data = {
    "image": None,
    "led_count": 0,
    "timestamp": 0,
    "quality_score": 0,
    "recent_images": deque(maxlen=150)  # Store ~5 seconds of images at 30fps
}
...
def calculate_image_quality_score(image, led_count):
    """Calculate a quality score for an image based on multiple factors"""
    if image is None or led_count == 0:
        return 0
    
    # Convert to grayscale for analysis
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    
    # Factor 1: LED count (primary factor)
    led_score = led_count * 10
    
    # Factor 2: Image sharpness (Laplacian variance)
    sharpness = cv2.Laplacian(gray, cv2.CV_64F).var()
    sharpness_score = min(sharpness / 100, 10)  # Normalize to 0-10
    
    # Factor 3: Brightness consistency (avoid over/under exposure)
    mean_brightness = np.mean(gray)
    brightness_score = 10 - abs(mean_brightness - 128) / 12.8  # Optimal around 128
    
    # Factor 4: Contrast (standard deviation of pixel values)
    contrast = np.std(gray)
    contrast_score = min(contrast / 10, 10)  # Normalize to 0-10
    
    # Weighted total score
    total_score = (led_score * 0.6 + sharpness_score * 0.2 + 
                   brightness_score * 0.1 + contrast_score * 0.1)
    
    return total_score

def update_best_image(image, total_leds):
    """Update the best image using enhanced quality assessment"""
    global best_image_data
    current_time = time.time()
    
    # Calculate quality score for this image
    quality_score = calculate_image_quality_score(image, total_leds)
    
    # Add to recent images with quality score
    ...
    # Clean up old images (older than 5 seconds)
    cutoff_time = current_time - 5.0
    while (best_image_data["recent_images"] and 
           best_image_data["recent_images"][0]["timestamp"] < cutoff_time):
        best_image_data["recent_images"].popleft()
    
    # Update best image if this one has better quality score
    if quality_score > best_image_data["quality_score"]:
        best_image_data["image"] = image.copy()
        best_image_data["led_count"] = total_leds
        best_image_data["quality_score"] = quality_score
    ...
    try:
        with contextlib.ExitStack() as stack:
            ...
            while camera_running:
                ...
                if len(frames) == 2:
                    #frame1 = frames["Camera 1"] if devicesID[0].startswith("144") else frames["Camera 2"]
                    #frame2 = frames["Camera 2"] if devicesID[1].startswith("184") else frames["Camera 1"]
...
def get_led_mode_color(led_id: str) -> Optional[str]:
    """Get the most frequent color for an LED based on samples from last 5 seconds with improved confidence"""
    ...
    # Filter samples to only include those from the last 5 seconds (increased from 3)
    for sample in led_color_samples[led_id]:
        if isinstance(sample, dict) and "timestamp" in sample:
            if current_time - sample["timestamp"] <= 5.0:  # Last 5 seconds for more stable detection
                recent_samples.append(sample["color"])
        else:
            # Handle old format samples (just color strings) - treat as expired
            continue
    
    # Require minimum samples for reliable detection (at least 5 frames)
    if len(recent_samples) < 5:
        return None  # Not enough samples for reliable detection
    ...
    max_count = color_counts[most_frequent_color]
    
    # Require at least 60% confidence (majority of samples must agree)
    confidence_threshold = len(recent_samples) * 0.6
    
    if max_count >= confidence_threshold and max_count > 0:
        return most_frequent_color
    else:
        # If no color has sufficient confidence, LED detection is unreliable
        return None
...
async def get_best_image_info():
    """Get information about the best image with quality metrics"""
    global best_image_data
    ...
    return {
        ...
        "quality_score": best_image_data["quality_score"],
        "timestamp": best_image_data["timestamp"],
        "age_seconds": current_time - best_image_data["timestamp"] if best_image_data["timestamp"] > 0 else 0,
        "recent_images_count": len(best_image_data["recent_images"]),
        "analysis_window_seconds": 5.0
    }

@app.get("/optimal_frame")
async def get_optimal_frame():
    """Get the optimal frame from recent captures using multi-frame analysis"""
    global best_image_data
    
    if not best_image_data["recent_images"]:
        raise HTTPException(status_code=404, detail="No recent frames available")
    
    current_time = time.time()
    
    # Filter recent images (last 3 seconds for optimal frame selection)
    recent_frames = [
        img for img in best_image_data["recent_images"] 
        if current_time - img["timestamp"] <= 3.0
    ]
    
    if not recent_frames:
        raise HTTPException(status_code=404, detail="No recent frames in analysis window")
    
    # Find frame with highest quality score
    optimal_frame = max(recent_frames, key=lambda x: x["quality_score"])
    
    # Convert frame to JPEG
    _, buffer = cv2.imencode('.jpg', optimal_frame["image"])
    
    return StreamingResponse(
        io.BytesIO(buffer.tobytes()),
        media_type="image/jpeg",
        headers={
            "Content-Disposition": "inline; filename=optimal_frame.jpg",
            "X-LED-Count": str(optimal_frame["led_count"]),
            "X-Quality-Score": str(round(optimal_frame["quality_score"], 2)),
            "X-Frame-Age": str(round(current_time - optimal_frame["timestamp"], 2))
        }
    )

@app.get("/optimal_frame_info")
async def get_optimal_frame_info():
    """Get information about the optimal frame selection process"""
    global best_image_data
    
    if not best_image_data["recent_images"]:
        return {"message": "No recent frames available", "frames_analyzed": 0}
    
    current_time = time.time()
    
    # Filter recent images (last 3 seconds)
    recent_frames = [
        img for img in best_image_data["recent_images"] 
        if current_time - img["timestamp"] <= 3.0
    ]
    ...
    if not recent_frames:
        return {"message": "No recent frames in analysis window", "frames_analyzed": 0}
    
    # Calculate statistics
    quality_scores = [frame["quality_score"] for frame in recent_frames]
    led_counts = [frame["led_count"] for frame in recent_frames]
    ...
    return {
        ...
        "optimal_frame": {
            "led_count": optimal_frame["led_count"],
            "quality_score": round(optimal_frame["quality_score"], 2),
            "age_seconds": round(current_time - optimal_frame["timestamp"], 2)
        },
        "statistics": {
            ...
            "avg_led_count": round(sum(led_counts) / len(led_counts), 1),
            "max_led_count": max(led_counts),
            "min_led_count": min(led_counts)
        }
    }
