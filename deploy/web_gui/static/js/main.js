...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Debug: Log the test controls card element
    setTimeout(() => {
        const testControlsCard = document.getElementById('testControlsCard');
        console.log('Test Controls Card found:', testControlsCard);
        console.log('Current classes:', testControlsCard ? testControlsCard.className : 'Element not found');
    }, 1000);
});
...
function setupEventListeners() {
    ...
    // Permanent input buttons
    document.getElementById('permanentYes').addEventListener('click', () => sendResponse('yes'));
    document.getElementById('permanentNo').addEventListener('click', () => sendResponse('no'));
    document.getElementById('permanentSkip').addEventListener('click', () => sendResponse('skip'));
...
function setupKeyboardShortcuts() {
    document.addEventListener('keydown', function(e) {
        ...
        // Handle button responses with permanent buttons
...
function updateTestStatus(status) {
    ...
    // Update Test Controls background based on status
    updateTestControlsBackground(status);
...
function updateTestControlsBackground(status) {
    const testControlsCard = document.getElementById('testControlsCard');
    
    if (!testControlsCard) {
        console.error('Test controls card not found');
        return;
    }
    
    // Remove all status classes
    testControlsCard.classList.remove('test-controls-ready', 'test-controls-running', 'test-controls-error', 'test-controls-success', 'test-controls-manual');
    
    let newClass = 'test-controls-ready'; // default
    
    // Determine status based on current step and running state
    if (status.running) {
        // Test is currently running
        newClass = 'test-controls-running';
        console.log('Setting test controls to running state');
    } else if (status.current_step) {
        // Test has finished - check the status
        const step = status.current_step.toLowerCase();
        console.log('Checking step:', step);
        
        // Check for manual inspection requirement (pink)
        if (step.includes('manual') || step.includes('verification') || step.includes('inspect') || step.includes('check')) {
            newClass = 'test-controls-manual';
            console.log('Setting test controls to manual inspection state (pink)');
        }
        // Check for success states (green)
        else if (step.includes('completed') || step.includes('success') || step.includes('pass')) {
            newClass = 'test-controls-success';
            console.log('Setting test controls to success state (green)');
        }
        // Check for error states (red)
        else if (step.includes('error') || step.includes('fail') || step.includes('timeout') || step.includes('connection')) {
            newClass = 'test-controls-error';
            console.log('Setting test controls to error state (red)');
        }
        // Default for other states
        else {
            newClass = 'test-controls-ready';
            console.log('Setting test controls to ready state (default)');
        }
    } else {
        // Default ready state
        newClass = 'test-controls-ready';
        console.log('Setting test controls to ready state (no step)');
    }
    
    // Add the new class
    testControlsCard.classList.add(newClass);
    console.log('Applied class:', newClass);
}
...
function showUserInputPanel(prompt, inputType = 'buttons') {
    ...
    // Show prompt text in the permanent buttons section
    const promptText = document.getElementById('inputPromptText');
    promptText.textContent = prompt;
    promptText.classList.remove('hidden');
    
    // Activate the permanent buttons with glow effect
    const buttons = document.querySelectorAll('.input-btn');
    buttons.forEach(button => {
        button.classList.add('active');
    });
    
    // Focus on text input if it's a text input type
    if (inputType === 'text') {
        setTimeout(() => {
    ...
    setTimeout(() => {
        document.getElementById('permanentYes').closest('.card').scrollIntoView({ 
            behavior: 'smooth', 
            block: 'center' 
        });
    }, 100);
...
function hideUserInputPanels() {
    ...
    // Deactivate the permanent buttons (remove glow effect)
    const buttons = document.querySelectorAll('.input-btn');
    buttons.forEach(button => {
